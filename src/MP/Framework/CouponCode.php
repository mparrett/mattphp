...
class CouponCode
{
    private static $BITCOUNT = 30;
    public static $BITMASK   = 32767; // (1 << (30 >> 1)) - 1
           public static $ALPHABET  = "XUF57ESCYK620D8QZNL3RHMG4W9PJTVA";
    public static $MAX       = 4294967296; // 2 << 31;
    ...
    public static function hash($num)
    {
        ...
        for ($round = 0; $round < 9; ++$round) {
            $left = $left ^ self::roundFunction($right); // Left XOR with round func
                   $temp = $left;
            $left = $right;
            $right = $temp; // Swap
        }
...
if (isset($argv) && $argc > 1) {
    ...
    for ($i=CouponCode::$MAX;$i>=0;$i--) {
        ...
        echo sprintf("%d\t%d\t%d\t%s\t%s\n",
            ...
            CouponCode::hash(CouponCode::codeFromCoupon(CouponCode::getCode($c)))
            );
    }
}
